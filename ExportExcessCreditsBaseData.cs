...
namespace Example_ExportExcessCreditsBaseData_
{
    ...
    public partial class ExportExcessCreditsBaseData : BaseForm
    {
        ...
        {            
            ...
            Utility.CompletedXls("外掛_學生會考報名檔", wb);
            this.Close();
        }
            ...
            foreach (string id in exportStudentIDList)
            {
                ...
                if (_StudentDict.ContainsKey(id))
                {
                    ...
                    if (_PhoneDict.ContainsKey(id))
                    {
                        if (strType6 == "戶籍電話")
                        {
                            dr["室內電話"] = ParseTelStr(_PhoneDict[id].Permanent);
                        }
                        if (strType6 == "聯絡電話")
                        {
                            dr["室內電話"] = ParseTelStr(_PhoneDict[id].Contact);
                        }
        ...
        {            
            if (_ConfigXML != null)
            {
                ...
                cboType6.Text = xmlParse1(elm, "電話");
                cboType7.Text = xmlParse1(elm, "家長");
